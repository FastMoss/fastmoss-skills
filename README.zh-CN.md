...
- 已安装 Node.js LTS。Node.js 会自带 `npm` 和 `npx`，建议使用 Node.js 18 或以上版本。
- 可以访问 npm registry，因为 `npx` 会按需下载 `@fastmoss/fastmoss-skills`。
- 使用的 AI 客户端支持本地 skill 目录。
- 有一个目标 skill 安装目录。新手可以直接使用下面的 `--client claude` 快捷命令；其他客户端可以使用 `--dest` 指定目录。
- 如果要进行实时 TikTok Shop 数据分析，还需要先安装并连接 FastMoss MCP。skill 本身可以先安装，但实时数据工具依赖 MCP 服务。
...
如果这些命令不存在，请先从 [nodejs.org](https://nodejs.org/) 安装 Node.js LTS。无需全局安装本 npm 包。
